    ...
    switch( event->type )
    {
        ...
        case ALLEGRO_EVENT_MOUSE_BUTTON_DOWN:
        {
            ...
            else if( event->mouse.button == 1 && wz_widget_rect_test( wgt, event->mouse.x, event->mouse.y ) )
            {
                ...
                box->cursor_pos =
                    wz_get_text_pos( font, (ALLEGRO_USTR *)text, event->mouse.x - wgt->x ) + box->scroll_pos;
            }
            else
                ret = 0;
        ...
        case ALLEGRO_EVENT_TOUCH_BEGIN:
        {
            ...
            else if( wz_widget_rect_test( wgt, event->touch.x, event->touch.y ) )
            {
                ...
                wz_ask_parent_for_focus( wgt );
                box->cursor_pos =
                    wz_get_text_pos( font, (ALLEGRO_USTR *)text, event->touch.x - wgt->x ) + box->scroll_pos;
            }
            else
                ret = 0;
...
*/
void wz_init_editbox( WZ_EDITBOX *box,
                      WZ_WIDGET *parent,
                      float x,
                      float y,
                      float width,
