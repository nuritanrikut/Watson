...
int draw_symbols( Game *g, Board *b )
{
    ...
    draw_horizontal_arrow(
        ( 3 * cus + 2 * b->clue_unit_space ) / 2, cus * 0.9, cus / 2, cus * 0.9, SYMBOL_COLOR, cus * 0.05 );
    draw_horizontal_arrow( ( 3 * cus + 2 * b->clue_unit_space ) / 2,
                           cus * 0.9,
                           ( 3 * cus + 2 * b->clue_unit_space ) - cus / 2,
                           cus * 0.9,
                           SYMBOL_COLOR,
                           cus * 0.05 );
    ...
    al_draw_line( cus * 0.975,
                  2.5 * cus + 2 * b->clue_unit_space,
                  cus * 0.975,
                  1.5 * cus + b->clue_unit_space,
                  SYMBOL_COLOR,
                  cus * 0.05 );
    draw_horizontal_arrow( cus,
                           ...
                           cus * 0.05 );
    draw_horizontal_arrow(
        cus, 1.5 * cus + b->clue_unit_space, cus * 0.7, 1.5 * cus + b->clue_unit_space, SYMBOL_COLOR, cus * 0.05 );
    ...
    draw_horizontal_arrow( 0,
                           ...
                           cus * 0.05 );
    draw_horizontal_arrow(
        0, 1.5 * cus + b->clue_unit_space, cus * 0.3, 1.5 * cus + b->clue_unit_space, SYMBOL_COLOR, cus * 0.05 );

    al_draw_filled_circle( cus / 2, 2 * cus + 1.5 * b->clue_unit_space, cus * 0.15, SYMBOL_COLOR );
    al_draw_line( cus / 2,
                  ( 2 - 0.08 ) * cus + 1.5 * b->clue_unit_space,
                  cus / 2,
                  ( 2 + 0.08 ) * cus + 1.5 * b->clue_unit_space,
                  WHITE_COLOR,
                  cus * 0.05 );
...
int draw_classic_symbols( Game *g, Board *b )
{
    ...
    b->symbol_bmp[SYM_SWAPPABLE] =
        al_create_bitmap( 3 * b->clue_unit_size + 2 * b->clue_unit_space, b->clue_unit_size );
    ...
    al_draw_scaled_bitmap(
        symbol_bmp[SYM_SWAPPABLE], 0, 0, 3 * 80, 80, 0, 0, 3 * b->clue_unit_size, b->clue_unit_size, 0 );
    ...
    al_clear_to_color( NULL_COLOR );
    al_draw_scaled_bitmap(
        symbol_bmp[SYM_ONLY_ONE], 0, 0, 80, 3 * 80, 0, 0, b->clue_unit_size, 3 * b->clue_unit_size, 0 );
        ...
        for( j = 0; j < b->n; j++ )
        {
            ...
            if( GLYPH_SHADOWS )
            {
                al_draw_glyph( tile_font1,
                               DARK_GREY_COLOR,
                               ( b->panel.b[0]->b[0]->w - bbw ) / 2 - bbx + 1,
                               ( b->panel.b[0]->b[0]->h - bbh ) / 2 - bby + 1,
                               CLUE_CODE[i][j][0] );
            }
            al_draw_glyph( tile_font1,
                           al_color_html( CLUE_FG_COLOR[i] ),
                           ( b->panel.b[0]->b[0]->w - bbw ) / 2 - bbx,
                           ( b->panel.b[0]->b[0]->h - bbh ) / 2 - bby,
                           CLUE_CODE[i][j][0] );
            ...
            if( TILE_SHADOWS )
                ...
                al_draw_rectangle(
                    .5, .5, b->panel.b[0]->b[0]->w - .5, b->panel.b[0]->b[0]->h - .5, TILE_GENERAL_BD_COLOR, 1 );
            ...
            if( GLYPH_SHADOWS )
            {
                al_draw_glyph( tile_font2,
                               DARK_GREY_COLOR,
                               ( b->panel_tile_size - bbw ) / 2 - bbx + 1,
                               ( b->panel_tile_size - bbh ) / 2 - bby + 1,
                               CLUE_CODE[i][j][0] );
            }
            al_draw_glyph( tile_font2,
                           al_color_html( CLUE_FG_COLOR[i] ),
                           ( b->panel_tile_size - bbw ) / 2 - bbx,
                           ( b->panel_tile_size - bbh ) / 2 - bby,
                           CLUE_CODE[i][j][0] );
            if( TILE_SHADOWS )
            ...
            if( GLYPH_SHADOWS )
            {
                al_draw_glyph( tile_font3,
                               DARK_GREY_COLOR,
                               ( b->clue_unit_size - bbw ) / 2 - bbx + 1,
                               ( b->clue_unit_size - bbh ) / 2 - bby + 1,
                               CLUE_CODE[i][j][0] );
            }
            al_draw_glyph( tile_font3,
                           al_color_html( CLUE_FG_COLOR[i] ),
                           ( b->clue_unit_size - bbw ) / 2 - bbx,
                           ( b->clue_unit_size - bbh ) / 2 - bby,
                           CLUE_CODE[i][j][0] );
            if( TILE_SHADOWS )
    ...
    for( i = 0; i < g->clue_n; i++ )
    {
        ...
        switch( g->clue[i].rel )
        {
            ...
            case NOT_MIDDLE:
                ...
                al_draw_bitmap(
                    b->clue_unit_bmp[g->clue[i].j[1]][g->clue[i].k[1]], b->clue_unit_size + b->clue_unit_space, 0, 0 );
                al_draw_bitmap( b->clue_unit_bmp[g->clue[i].j[2]][g->clue[i].k[2]],
                                2 * ( b->clue_unit_size + b->clue_unit_space ),
                                0,
                                0 );
                if( g->clue[i].rel == NOT_NEXT_TO )
                {
                    ...
                    al_draw_bitmap(
                        b->symbol_bmp[SYM_FORBIDDEN], 2 * ( b->clue_unit_size + b->clue_unit_space ), 0, 0 );
                }
            ...
            case ONE_SIDE:
                ...
                al_draw_bitmap( b->clue_unit_bmp[g->clue[i].j[1]][g->clue[i].k[1]],
                                2 * ( b->clue_unit_size + b->clue_unit_space ),
                                0,
                                0 );
                break;
            ...
            case NOT_TOGETHER:
                al_draw_bitmap( b->clue_unit_bmp[g->clue[i].j[0]][g->clue[i].k[0]], 0, 0, 0 );
                al_draw_bitmap(
                    b->clue_unit_bmp[g->clue[i].j[1]][g->clue[i].k[1]], 0, b->clue_unit_size + b->clue_unit_space, 0 );
                if( ( g->clue[i].rel == NOT_TOGETHER ) || ( g->clue[i].rel == TOGETHER_NOT_MIDDLE ) )
                    ...
                    al_draw_bitmap(
                        b->symbol_bmp[SYM_ONLY_ONE], 0, 0, 0 ); //xxx todo: temporary  -- add ONLY_ONE symbol
                break;
...
void show_info_text( Board *b, ALLEGRO_USTR *msg )
{
    ...
    al_draw_multiline_ustr( b->text_font,
                            INFO_TEXT_COLOR,
                            10,
                            3,
                            b->info_panel.w - 2 * al_get_font_line_height( b->text_font ),
                            al_get_font_line_height( b->text_font ),
                            ALLEGRO_ALIGN_LEFT,
                            msg );
...
int update_bitmaps( Game *g, Board *b )
{
    ...
    if( !( b->text_font =
               load_font_mem( text_font_mem,
                              TEXT_FONT_FILE,
                              -min( b->info_panel.h / 2.2, sqrt( b->info_panel.w * b->info_panel.h ) / 10 ) ) ) )
    {
    ...
    for( i = 0; i < 4; i++ )
    {
        b->button_bmp_scaled[i] =
            scaled_clone_bitmap( b->button_bmp[i], b->time_panel.b[i + 1]->w, b->time_panel.b[i + 1]->h );
    }
    ...
    for( i = 0; i < b->h; i++ )
    {
        for( j = 0; j < b->n; j++ )
        {
            ...
            al_draw_scaled_bitmap( basic_bmp[i][j],
                                   ...
                                   ( b->panel.b[0]->b[0]->w - s ) / 2,
                                   ( b->panel.b[0]->b[0]->h - s ) / 2,
                                   s,
                                   s,
                                   0 );
...
void draw_title( void )
{
    ...
    for( w = 0; w < 150; w++ )
    {
        al_draw_filled_rectangle(
            w + dw, w / 2 + dh, 2 * dw - w + dw, 2 * dh - w / 2 + dh, al_map_rgba_f( 0.03, 0.01, 0.01, 0.04 ) );
    }
...
ALLEGRO_BITMAP *get_clue_bitmap( Board *b, Clue *clue )
{
    ...
    switch( clue->rel )
    {
        ...
        case NOT_MIDDLE:
            ...
            al_draw_bitmap(
                b->clue_unit_bmp[clue->j[2]][clue->k[2]], 2 * ( b->clue_unit_size + b->clue_unit_space ), 0, 0 );
        ...
        case ONE_SIDE:
            ...
            al_draw_bitmap( b->symbol_bmp[SYM_ONE_SIDE], b->clue_unit_size + b->clue_unit_space, 0, 0 );
            al_draw_bitmap(
                b->clue_unit_bmp[clue->j[1]][clue->k[1]], 2 * ( b->clue_unit_size + b->clue_unit_space ), 0, 0 );
            break;
        case TOGETHER_3:
        case TOGETHER_NOT_MIDDLE:
            al_draw_bitmap(
                b->clue_unit_bmp[clue->j[2]][clue->k[2]], 0, 2 * ( b->clue_unit_size + b->clue_unit_space ), 0 );
        case TOGETHER_2:
        case NOT_TOGETHER:
            al_draw_bitmap( b->clue_unit_bmp[clue->j[0]][clue->k[0]], 0, 0, 0 );
...
void create_font_symbols( Board *b )
{
    ...
    for( j = 0; j < b->h; j++ )
    {
        for( i = 0; i < b->n; i++ )
        {
            ...
            al_draw_scaled_bitmap(
                b->clue_unit_bmp[j][i], 0, 0, bw, bh, 2 + i * ( 2 + nbw ), 2 + j * ( 2 + nbh ), nbw, nbh, 0 );
    ...
    for( i = 0; i < b->n; i++ )
    {
        ...
        al_draw_scaled_bitmap(
            b->button_bmp[i % 4], 0, 0, bw, bh, 2 + i * ( 2 + nbw ), 2 + j * ( 2 + nbh ), nbw, nbh, 0 );
        al_draw_rectangle( 2 + i * ( 2 + nbw ) + 0.5,
                           2 + j * ( 2 + nbh ) + 0.5,
                           2 + i * ( 2 + nbw ) + nbw - 0.5,
                           2 + j * ( 2 + nbh ) + nbh - 0.5,
