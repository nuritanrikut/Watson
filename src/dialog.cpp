...
int yes_no_dialog( const char *text )
{
    ...
    al_draw_filled_rectangle(
        ( dw - bw ) / 2, ( dh - bh ) / 2, ( dw + bw ) / 2, ( dh + bh ) / 2, WINDOW_BACKGROUND_COLOR );
    al_draw_rectangle( ( dw - bw ) / 2, ( dh - bh ) / 2, ( dw + bw ) / 2, ( dh + bh ) / 2, WINDOW_BORDER_COLOR, 3 );

    al_draw_multiline_text(
        default_font, al_map_rgb_f( 1, 1, 1 ), dw / 2, ( dh - bh ) / 2 + 20, 360, 18, ALLEGRO_ALIGN_CENTER, text );
    ...
    while( noexit )
    {
        ...
        {
            if( ( ev.mouse.x > yes_x ) && ( ev.mouse.x < yes_x + 64 ) && ( ev.mouse.y > yes_y )
                && ( ev.mouse.y < yes_y + 24 ) )
            {
                ret = 1;
                noexit = 0;
            }
            else if( ( ev.mouse.x > no_x ) && ( ev.mouse.x < no_x + 64 ) && ( ev.mouse.y > no_y )
                     && ( ev.mouse.y < no_y + 24 ) )
            {
                ret = 0;
                noexit = 0;
