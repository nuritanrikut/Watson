...
{ // note that b->vclue.b and v->hclue.b are destroyed elsewhere
    int i, j;
...
int create_board( Game *g, Board *b, int mode )
{
    ...
    panel_tile_w = ( ( b->panel.w - ( b->n - 1 ) * PANEL_COLUMN_SPACE ) / b->n - 2 * PANEL_BLOCK_MARGIN
                     - ( PANEL_TILE_COLUMNS[b->n] - 1 ) * PANEL_TILE_SPACE )
                   / ( PANEL_TILE_COLUMNS[b->n] );
    panel_tile_h = ( b->panel.h - b->h * ( ( PANEL_TILE_ROWS[b->n] - 1 ) * PANEL_TILE_SPACE + 2 * PANEL_BLOCK_MARGIN ) )
                   / ( b->h * PANEL_TILE_ROWS[b->n] ); //ok go
    ...
    // panel columns
    if( mode )
        b->panel.b = malloc( b->n * sizeof( struct TiledBlock * ) );
    for( i = 0; i < b->n; i++ )
    {
        if( mode )
        {
        ...
        for( j = 0; j < b->h; j++ )
        {
            ...
            b->panel.b[i]->b[j]->h =
                PANEL_TILE_ROWS[b->n] * panel_tile_h + ( PANEL_TILE_ROWS[b->n] - 1 ) * PANEL_TILE_SPACE;
            b->panel.b[i]->b[j]->x = PANEL_BLOCK_MARGIN;
            b->panel.b[i]->b[j]->y =
                j * ( b->panel.b[i]->b[j]->h + block_space + 2 * PANEL_BLOCK_MARGIN ) + PANEL_BLOCK_MARGIN;
    ...
    cus = min( b->hclue.w - 2 * CLUE_TILE_MARGIN - 2 * CLUE_UNIT_SPACE,
               b->vclue.h - 2 * CLUE_TILE_MARGIN - 2 * CLUE_UNIT_SPACE )
          / 3;
    cus = min( cus,
               ( b->panel.h + 2 * CLUE_UNIT_SPACE + 2 * CLUE_TILE_MARGIN + VCLUEBOX_MARGIN + b->panel.margin
                 - 2 * CLUE_TILE_MARGIN * b->hclue_n )
                   / ( max( b->hclue_n - 3, 1 ) ) );
    ...
    b->info_panel.y = b->vclue.y + b->vclue.h + b->vclue.margin + INFO_PANEL_MARGIN
                      + min( ( b->max_ysize - b->ysize - b->info_panel.h - 2 * INFO_PANEL_MARGIN ) / 2, 30 );
    ...
    b->info_panel.w =
        b->hclue.x - b->hclue.margin - INFO_PANEL_MARGIN - b->panel.x; // b->vclue.w; // assumes margins are equal
    ...
    { // buttons
        ...
        b->time_panel.b[i + 1]->y =
            ( ( b->time_panel.h + ( b->time_panel.b[0]->y + b->time_panel.b[0]->h ) ) - b->time_panel.b[i + 1]->h ) / 2;
        b->time_panel.b[i + 1]->x =
            ( ( i + 1 ) * b->time_panel.w + ( i - TIME_PANEL_BUTTONS ) * b->time_panel.b[i + 1]->w )
