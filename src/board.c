...
int create_board( Game *game, Board *board, int mode )
{
    ...
    if( mode )
    {
        board->clue_bmp = malloc( game->clue_n * sizeof( *board->clue_bmp ) );
        board->clue_tiledblock = malloc( game->clue_n * sizeof( *board->clue_tiledblock ) );
    }
    ...
    // panel columns
    if( mode )
        board->panel.sub = malloc( board->number_of_columns * sizeof( struct TiledBlock * ) );
    for( i = 0; i < board->number_of_columns; i++ )
    {
        if( mode )
        {
            board->panel.sub[i] = malloc( sizeof( struct TiledBlock ) );
            ...
            board->panel.sub[i]->sub = malloc( board->column_height * sizeof( struct TiledBlock * ) );
        ...
        for( j = 0; j < board->column_height; j++ )
        {
            if( mode )
            {
                board->panel.sub[i]->sub[j] = malloc( sizeof( struct TiledBlock ) );
                ...
                board->panel.sub[i]->sub[j]->sub = malloc( board->number_of_columns * sizeof( struct TiledBlock * ) );
            ...
            for( k = 0; k < board->number_of_columns; k++ )
            {
                if( mode )
                {
                    board->panel.sub[i]->sub[j]->sub[k] = malloc( sizeof( struct TiledBlock ) );
    ...
    //create Vclue tiles
    board->vclue.sub = malloc( board->vclue.number_of_subblocks * sizeof( struct TiledBlock * ) );
    for( i = 0; i < board->vclue.number_of_subblocks; i++ )
    {
        board->vclue.sub[i] = malloc( sizeof( struct TiledBlock ) );
    ...
    //create Hclue tiles
    board->hclue.sub = malloc( board->hclue.number_of_subblocks * sizeof( struct TiledBlock * ) );
    for( i = 0; i < board->hclue.number_of_subblocks; i++ )
    {
        board->hclue.sub[i] = malloc( sizeof( struct TiledBlock ) );
    ...
    { // if board is being created
        board->time_panel.sub = malloc( 6 * sizeof( struct TiledBlock * ) );
        for( i = 0; i < 5; i++ )
            board->time_panel.sub[i] = malloc( sizeof( struct TiledBlock ) );
    }
    ...
    { // only if board is being created
        ...
        board->all.sub = malloc( board->all.number_of_subblocks * sizeof( struct TiledBlock * ) );
